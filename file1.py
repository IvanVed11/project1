...
print('''Вы были на экскурсии в научном центре. Был солнечный морозный день, ничего не предвещало беды. 
      ...
      За каждую ошибку у вас снимается определенное количество xp. Изначально у вас 100xp
      Приступим!''')
game_cycle = True
while xp > 0:
    ...
    print('Перед каждой дверью будет вопрос по математике. Вопросов будет 7, приступим.')
    print()
    ...
    print()
    print("Количество xp:", xp)
    print('''Перед вами вторая дверь. Открыть ее будет сложнее чем предыдущую.''')
    print()
    ...
    print("Количество xp:", xp)
    print()
    print("У Вас появляется чувство голода. Вы хотите есть, а надо решить еще 4 задачи. Поторопитесь!")
    ...
    print("Количество xp:", xp)
    print()
    print('Урааа! Скоро Вы окажитесь дома, попъете чай и посидите в любимом кресле. Или нет?')
    ...
    print('''Вы находитесь перед выходом из здания, уже слышен щебет птиц и в помещние проникают
          ...
          этого страшного места. Будет рандомное число попыток. Что же, давайте быстрее начнем!''')
    print()
    print('Количество xp:', xp)
    print()
...
if xp > 0:
    print("Поздравляем с прохождением квеста",
         surname, name, 'ученик школы', school, 'города', city, class_number, 
         "класса смог сохранить xp на уровне > 0. Двери разблокированы")  
else:
    print("Система не может разблокировать двери, т к вы не человек. Кто Вы? Вы проиграли")